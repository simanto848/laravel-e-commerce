...
@section('admin_page_title')
    Manage Default Product Attribute - Admin Panel
@endsection
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">All Default Product Attribute</h5>
                </div>
                @if ($errors->any())
                    <div
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($defaultAttributes as $defaultAttribute)
                                    <tr>
                                        <td>{{ $defaultAttribute->id }}</td>
                                        <td>{{ $defaultAttribute->attribute_value }}</td>
                                        <td style="white-space: nowrap;">
                                            <a href="{{ route('default.product.attribute.show', $defaultAttribute->id) }}"
                                                class="btn btn-primary btn-sm"
                                                style="display: inline-block; margin-right: 0.5rem;">Edit</a>
                                            <form
                                                action="{{ route('default.product.attribute.delete', $defaultAttribute->id) }}"
